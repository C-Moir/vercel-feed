...
function loadState() {
  try {
    ...
    return JSON.parse(raw).lastSeenCrtShId || 0;
  } catch (_) {
    return 0;
  }
}
...
function saveState(id) {
  try {
    fs.writeFileSync(STATE_FILE, JSON.stringify({ lastSeenCrtShId: id }));
  } catch (_) {}
}
...
let lastSeenCrtShId = loadState();
console.log(`[crt.sh] resuming from cert ID ${lastSeenCrtShId}`);
